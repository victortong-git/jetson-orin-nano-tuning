...
if [ -f /etc/nv_tegra_release ] ; then
    JETPACK_VERSION=$(grep -o 'R[0-9]*' /etc/nv_tegra_release | head -1)
    case "$JETPACK_VERSION" in
        R39|R38)
            status "L4T R39/R38 (JetPack 6) detected"
            download_and_extract "https://ollama.com/download" "$OLLAMA_INSTALL_DIR" "ollama-linux-${ARCH}-jetpack6"
            ;;
        R37)
            status "L4T R37 detected"
            download_and_extract "https://ollama.com/download" "$OLLAMA_INSTALL_DIR" "ollama-linux-${ARCH}-jetpack7"
            ;;
        R36)
            status "L4T R36 detected"
            download_and_extract "https://ollama.com/download" "$OLLAMA_INSTALL_DIR" "ollama-linux-${ARCH}-jetpack6"
            ;;
        R35)
            status "L4T R35 detected"
            download_and_extract "https://ollama.com/download" "$OLLAMA_INSTALL_DIR" "ollama-linux-${ARCH}-jetpack5"
            ;;
        R34)
            status "L4T R34 (JetPack 5) detected"
            download_and_extract "https://ollama.com/download" "$OLLAMA_INSTALL_DIR" "ollama-linux-${ARCH}-jetpack5"
            ;;
        R33|R32)
            status "L4T R33/R32 (JetPack 4) detected"
            download_and_extract "https://ollama.com/download" "$OLLAMA_INSTALL_DIR" "ollama-linux-${ARCH}-jetpack4"
            ;;
        *)
            # Unknown L4T version – try generic jetpack binary, fall back gracefully
            warning "Unknown L4T version '$JETPACK_VERSION'. Attempting generic jetpack download..."
            download_and_extract "https://ollama.com/download" "$OLLAMA_INSTALL_DIR" "ollama-linux-${ARCH}-jetpack6" || \
            download_and_extract "https://ollama.com/download" "$OLLAMA_INSTALL_DIR" "ollama-linux-${ARCH}-jetpack5" || \
            warning "No compatible JetPack binary found. GPU may not be supported."
            ;;
    esac
fi
...
configure_systemd() {
    ...
    if getent group nvidia >/dev/null 2>&1; then
        status "Adding ollama user to nvidia group..."
        $SUDO usermod -a -G nvidia ollama
    fi

    # Enable jetson_clocks on Jetson platforms for max performance
    if [ -f /etc/nv_tegra_release ] && [ -f /usr/bin/jetson_clocks ]; then
        status "Enabling jetson_clocks for maximum GPU performance..."
...
if [ -f /etc/nv_tegra_release ] ; then
    # Add current user to the nvidia group for GPU access on Jetson
    if getent group nvidia >/dev/null 2>&1; then
        $SUDO usermod -a -G nvidia $(whoami) 2>/dev/null || true
    fi

    # Ensure JETSON_JETPACK=6 is set in the ollama service so GPU is used
    if [ -f /etc/systemd/system/ollama.service ] && \
       ! grep -q 'JETSON_JETPACK' /etc/systemd/system/ollama.service 2>/dev/null; then
        status "Adding JETSON_JETPACK=6 to ollama service for GPU support..."
        $SUDO sed -i '/^Environment="PATH/a Environment="JETSON_JETPACK=6"' /etc/systemd/system/ollama.service
